...
<html lang="en" dir="ltr">
  ...
  <body>
    <form class="box" role="form" method="POST" action="{{ route('usuario') }}">
        @csrf
        <h1>Inicio de Sesion</h1>
        <input class="form-control{{ $errors->has('login') ? ' is-invalid' : '' }}" placeholder="{{ __('Nombre de Usuario') }}" type="text" name="login" value="{{ old('login') }}" required autofocus>
        @if ($errors->has('login'))
